...
def main():
    ...
    df = pandas.read_excel(os.path.join(os.path.dirname(__file__), fileName),
                           ...
                           converters={'FN_Fn06002':
                                           lambda s:
                                           # Money(str(s),Currency.CNY).sub_units
                                           100 * int(str(s).split('.')[0])
                                                     + int(str(s).split('.')[1].ljust(2, '0')
                                                           if '.' in str(s) else 0)
                                           if float(s)>0
                                           else 100 * int(str(s).split('.')[0])
                                                 - int(str(s).split('.')[1].ljust(2, '0')
                                                       if '.' in str(s) else 0)
                                       },
    ...
    # print(df)
    df.to_excel("F:\python\大创\数据V5.xlsx", sheet_name='Sheet1')
